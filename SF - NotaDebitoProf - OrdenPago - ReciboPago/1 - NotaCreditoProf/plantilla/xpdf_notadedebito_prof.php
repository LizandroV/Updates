...
class PDF_Code128 extends FPDF
{
    ...
    function CabeceraPrincipal()
    {
        ...
        $codigoNDProf = base64_decode(trim($_REQUEST['codndprof']));
        ...
        $sql_cabecera = "SELECT C.CodNotaProf, C.FecReg,
						...
						WHERE C.Estado not in ('C') and C.CodOrdNotaDeb='$codigoNDProf' and CodNotaNeg='$codigoNeg'";
        // echo $sql_cabecera;
        ...
        $this->Cell(180, 7, 'NOTA DE DEBITO DE PROFORMA ', 0, 1, 'C');
        $this->Cell(180, 7, 'N.D. ' . (string)str_pad($codigoNDProf, 7, '0', STR_PAD_LEFT) . '-' . $negcne, 0, 0, 'C');
        $this->Cell(110);
    ...
    function FancyTable($header)
    {
        ...
        $sql  = "SELECT D.Glosa, D.Cantidad, D.Punitario, D.Monto, M.MedAbrev
				FROM CABNOTADEBITO_PROF C
				LEFT JOIN DETNOTADEBITO_PROF D ON C.CodOrdNotaDeb = D.CodOrdNotaDeb AND C.CodNotaNeg = D.CodNotaNeg
				LEFT JOIN MEDIDA M ON M.MedCod = D.MedCod
				WHERE C.Estado not in ('C') AND C.CodOrdNotaDeb = '$codigoNDProf' AND C.CodNotaNeg = '$codigoNeg'";
        ...
        if (strlen($comentario) > 0) {
            ...
            $this->Cell(40, 4, 'COMENTARIOS ND:', 0, 1, 'L', 0);
            $this->SetFont('Arial', '', 6);
...
$pdf->Output('DOC_ND_PROF' . $nombrefile . '.pdf', 'D');
