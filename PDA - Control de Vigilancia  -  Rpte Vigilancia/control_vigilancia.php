...
<form action="control_vigilancia.php" method="POST" enctype="multipart/form-data" name="formbuscar" id="formbuscar">
    ...
    if ($cotiza !== "") {
        ...
        switch ($tipo_doc) {
            case 'PT':
                $query = "select a.CodPL, convert(varchar(10), a.FechaReg,103)as FechaReg, a.Total as TotalKg, b.CliRaz, b.CliRaz, emp.EmpRaz, 
                ...
                break;
            case 'GR':
                $query = "select d.GuiaDespacho, convert(varchar(10), d.FecReg,103)as FechaReg, e.EmpRaz, c.CliRaz,
                ...
                break;
            case 'FT':
                $query = "select f.Factura, convert(varchar(10),f.FecReg,103)as FechaReg, e.EmpRaz, c.CliRaz, 
            ...
            case 'BV':
                ...
                break;
            case 'PF':
                $query = "select p.Proforma, convert(varchar(10),p.FecReg,103)as FechaReg, e.EmpRaz, c.CliRaz, 
            ...
            case 'GD':
                ...
                break;
            case 'PL':
                $query = "select a.CodPL, convert(varchar(10), a.FechaReg,103)as FechaReg, a.Total as TotalKg, b.CliRaz, b.CliRaz, 
                ...
                break;
            case 'NC':
                $query = "select f.NotaCredito, convert(varchar(10),f.FecReg,103)as FechaReg, e.EmpRaz, c.CliRaz, 
    ...
    <thead><tr>
        ...
        if ($tipo_doc == 'FT' || $tipo_doc == 'BV' || $tipo_doc == 'PT' || $tipo_doc == 'GR' || $tipo_doc == 'GD' || $tipo_doc == 'PL') {
            echo '<th><b>Cantidad</b></th>';
        };
        if ($tipo_doc == 'FT' || $tipo_doc == 'BV' || $tipo_doc == 'PF' || $tipo_doc == 'NC') {
            echo '<th><b>Monto</b></th>';
        };
        ...
        //AGREGAR RESULTADOS
        if ($tipo_doc == 'PT' || $tipo_doc == 'PL') {
            $nro_docu = htmlentities('P.L. N° ') . (string)str_pad(trim($rows['CodPL']), 7, '0', STR_PAD_LEFT);
        };
        if ($tipo_doc == 'GR' || $tipo_doc == 'GD') {
            $nro_docu = trim($rows['GuiaDespacho']);
            $medida_cant = '&nbsp;';
            $medida_kg = trim($rows['medida']);
        };
        if ($tipo_doc == 'FT' || $tipo_doc == 'BV') {
            $nro_docu = trim($rows['Factura']);
            $medida_cant = '&nbsp;';
            $medida_kg = trim($rows['medida']);
            $moneda = trim($rows['moneda']);
        };
        if ($tipo_doc == 'PF') {
            $nro_docu = trim($rows['Proforma']);
            $medida_cant = '&nbsp;';
            $medida_kg = trim($rows['medida']);
            $moneda = trim($rows['moneda']);
        };
        if ($tipo_doc == 'NC') {
            $nro_docu = trim($rows['NotaCredito']);
            $medida_cant = '&nbsp;';
            $medida_kg = trim($rows['medida']);
            $moneda = trim($rows['moneda']);
        };
        ...
        if ($tipo_doc == 'FT' || $tipo_doc == 'BV' || $tipo_doc == 'PT' || $tipo_doc == 'GR' || $tipo_doc == 'GD' || $tipo_doc == 'PL') {
            echo "<td data-label='Cantidad'>" . $rows['cantidad'] . " " . $medida_cant . "</td>";
        };
        if ($tipo_doc == 'FT' || $tipo_doc == 'BV' || $tipo_doc == 'PF' || $tipo_doc == 'NC') {
            echo "<td data-label='Monto'>" . $moneda . " " . number_format($rows['MontTotal'], 2) . "</td>";
        };

        echo "<td data-label='Total'>" . $rows['TotalKg'] . " " . $medida_kg . "</td>";
        echo "<td data-label='Adjuntar' height='50' align='center'>
                <label for='img_adjunta' class='botonNuevo' style='cursor: pointer; display: inline-flex; align-items: center; gap: 6px;'>
                    📷 Foto
                </label>
                <input type='file' id='img_adjunta' name='img_adjunta' accept='image/*' capture='camera' style='display: none;'>
            </td>";
